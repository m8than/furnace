...
- **Precision:** the existing gfx942 loader converts MXFP8 weights to `[128,128]` block FP8. This is not native MXFP8 execution or preservation of the original `1×32` scales. Activations and the index cache remain BF16, the main KV cache is FP8, routing stays FP32, and communication is unquantized. The tested checkpoint has no calibrated main-KV scales, so the cache uses unit scales.
