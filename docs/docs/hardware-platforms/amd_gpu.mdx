...
The gfx942 path avoids expert sorting for single-token MoE execution and uses K3 TP8 stage-specific tiles. ROCm MLA also skips redundant multiplication of BF16 weights by a scalar unit scale. For eligible small-batch, 512-dimensional MLA decode, the split merge runs across parallel value tiles with FP32 accumulation; larger batches retain the serial merge.
