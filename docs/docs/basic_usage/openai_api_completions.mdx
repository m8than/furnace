...
Streaming text responses send `X-Accel-Buffering: no` and
`Cache-Control: no-cache, no-transform` so supporting reverse proxies forward
updates without buffering them. If a proxy overrides these headers, disable
response buffering and compression for the streaming route (for example,
`proxy_buffering off;` and `gzip off;` in Nginx). Clients must also consume the
response incrementally; use `flush=True` when printing chunks to a terminal.

Keep `--stream-interval 1` (the default) for the smoothest delivery. Speculative
decoding can still emit several accepted tokens per update. These delivery
settings do not change sampling or speculative acceptance.
