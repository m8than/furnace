...
MiniMax-M3 also supports owner-local Triton dense and sparse attention with DCP. Its indexer combines global block scores before sparse selection; the main attention output uses the same FP32 LSE merge. See the [MiniMax-M3 ROCm preset](/cookbook/autoregressive/MiniMax/MiniMax-M3#fp8-kv-with-decode-context-parallelism-on-mi325x) for FP8 KV, precision boundaries, and workload-specific caveats.
...
<Warning>
MiniMax-M3 validates this stronger containment condition at configuration time. Other model paths may check only `tp_size % dcp_size == 0`; their DPA+DCP deployments must enforce containment when choosing the topology.
</Warning>
