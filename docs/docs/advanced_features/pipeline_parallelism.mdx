...
MiniMax-M3 supports DSpark with pipeline parallelism. The draft model runs on the last pipeline stage; the target stages transport the captured target features and verify the same proposed token block. Accepted tokens and committed sequence lengths are propagated back to every stage.

This configuration requires:

- `--disable-overlap-schedule`
- `SGLANG_RAGGED_VERIFY_MODE=static`
- Standalone serving, without DP attention or context parallel attention
- `--pp-async-batch-depth 0`

For a four-stage target with an unquantized draft checkpoint:

```bash Command
SGLANG_RAGGED_VERIFY_MODE=static python3 -m sglang.launch_server \
  --model-path "$TARGET_MODEL" \
  --tp-size 1 --pp-size 4 \
  --disable-overlap-schedule --pp-async-batch-depth 0 \
...
Set `TARGET_MODEL` and `DRAFT_MODEL` to the matching MiniMax-M3 target and DSpark checkpoint paths. The draft and its KV cache consume memory on the last stage, so check the resolved token capacity rather than assuming it matches a target-only launch.

The checkpoint determines the proposal block size unless `--speculative-dspark-block-size` overrides it. A block size of four proposes four draft tokens and verifies five tokens including the current token; its maximum acceptance length is five. Report the block size and prompt thinking mode alongside acceptance measurements: disabled-thinking prompts and adaptive-thinking chat prompts are different workloads.
