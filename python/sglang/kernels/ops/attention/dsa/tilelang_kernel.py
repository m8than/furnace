...
def sparse_mla_fwd_decode_partial(
    ...
    *,
    ...
    D = dim
    D_tail = tail_dim
    # NoPE has no rotary contribution. Do not materialize zero-width buffers:
    # TileLang layout/copy lowering otherwise divides by their zero extent.
    has_tail = tail_dim > 0
    ...
    if _is_hip and _q_in_shared:
        # Include Q, KV, probabilities, and aligned reduction scratch in LDS.
        shared_bytes = (
            2 * (H_per_block + BI) * (D + D_tail) + 2 * H_per_block * BI + 4096
        )
        shared_limit = torch.cuda.get_device_properties(
            torch.cuda.current_device()
        ).shared_memory_per_block
        _q_in_shared = shared_bytes <= shared_limit
